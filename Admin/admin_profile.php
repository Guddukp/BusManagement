...
<script>
  $(function(){
    $("#fileupload").change(function(event) {
      ...
      <!-- END Images Search -->
<?php
if(isset($_POST['upbtn']) && !empty($_FILES["file"]["name"])){
  ...
  $allowTypes = array('jpg','png','jpeg','gif','pdf');
    if(in_array($fileType, $allowTypes)){
        ...
        if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){
            ...
            $query="UPDATE `admin` SET `img`='$fileName',`name`='$name',`email`='$email',`phone`='$phone',`birthday`='$birthdate',`age`=$age,`city`='$city',`state`='$state' WHERE id={$_SESSION['id']}";
